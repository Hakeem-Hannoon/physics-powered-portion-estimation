...
4. 🟡 **On-device nutrient bundle** — **done for the demo**: `ExpoSqliteNutrientStore` over expo-sqlite reads a bundled SQLite DB (`apps/demo/assets/nutrients.sqlite`), with the curated label→FDC alias map (`FOOD_ALIASES`). A 12-food **starter** bundle ships now (`nutrition/starter/build-starter.mjs`); the full ~15k-food FDC bundle (`npm run etl:bundle`) is the remaining step.
