...
4. ✅ **On-device nutrient bundle — the full FDC database ships.** `ExpoSqliteNutrientStore` over expo-sqlite reads `apps/demo/assets/nutrients.sqlite`: **13.7k generic USDA foods** (Foundation + SR Legacy + FNDDS, ~55% with portion-derived density, FTS-indexed, 2.3 MB) with the curated 58-food set **overlaid** so the classifier vocabulary keeps its hand-checked rows, per-food **shape classes**, and the label→FDC map (`nutrition/label-map.json`). Built by `nutrition/starter/build-full.mjs` (`npm run build:nutrients:full`); the density ETL now parses **all three** real FDC portion encodings (measure ids / SR `modifier` text / FNDDS `portion_description`) — verified 176/176 checks (every vocab label + alias resolves to its curated row; real-FDC lookups work). Remaining: fitted **per-class** κ/φ/h̄ (needs per-class labels) and re-pointing the label map at verbatim USDA rows (the "class taxonomy" curation pass).
