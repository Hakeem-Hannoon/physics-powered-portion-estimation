...
- 🟡 **Per‑food shape class — mechanism done.** Each food row now carries a `shape_class` (`build-bundle.mjs` `foodClasses`), and both stores resolve it to that class's prior (falling back to `_global`), with per‑class **defaults** seeded for `mound`/`flat`/`container` (MATH.md §4) — e.g. bread and salmon use the `flat` slab route, not the mound prior. What's still pending is the **fitted** per‑class κ/φ/h̄ from [[Shape Priors and Nutrition5k]] (needs per‑class labels); when `fit_priors.py` emits them they override the defaults.
- ✅ **Label → FDC‑row map — real artifact shipped.** `nutrition/label-map.json` (copied next to the demo DB by `npm run build:nutrients`) is the curated `aliases` map — terse labels + synonyms → FDC descriptions. Extending it to the full FoodSeg103 classifier vocabulary is the remaining step (awaits the fixed vocab).
