...
1. **Median anchoring from the pre-touch buffer.** A rolling window of recent reticle hits (6 frames) is kept at all times; **press** anchors instantly on the component-wise median of that buffer — frames captured *before* the finger jolt existed, so the jolt cannot contaminate them. The live endpoint is the rolling median of the same window, and **release** commits it — the pre-lift reading, for the same reason. Zero-mean tremor shrinks roughly like $1/\sqrt{N}$, and the median — unlike a mean — ignores outlier spikes entirely.
2. **Shake gating, for violent motion only.** Per-frame camera velocities from pose deltas — linear $v = \lVert\Delta\mathbf{c}\rVert/\Delta t$, angular $\omega = 2\arccos\lvert\langle q_{t-1}, q_t\rangle\rvert / \Delta t$ — exclude samples beyond ~1.0 m/s or ~1.5 rad/s (fast swings, hard knocks). The device's measured motion on all axes serves as a *quality gate*; subtracting it from the measurement would double-count what VIO already corrected. Field lesson encoded here: gate thresholds near ordinary hand-sway levels (~0.3 rad/s) starve the buffers and block measuring altogether — tremor removal belongs to the median, the gate exists only for motion blur.
